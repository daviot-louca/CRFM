  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm h-[720px] flex flex-col overflow-hidden">
      <header className="px-5 py-5 border-b flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Groupes</h2>
                ...
                <div className="space-y-3">
                  ...
                  {groupe.automatique ? (
                    ...
                    <input
                      type="text"
                      value={groupe.nom}
                      onChange={(e) => renommerGroupe(index, e.target.value)}
                  ...
                  >
                    {groupe.automatique ? 'Supprimer le groupe automatique' : 'Supprimer le groupe'}
                  </button>
                </div>
                <div>
                  ...
                  <div className="max-h-72 overflow-y-auto space-y-4 pr-1">
                    {usersDisponibles.map((user) => {
                      const fullName = getNomComplet ? getNomComplet(user) : user.nom;
                      const selected = groupe.users.includes(user.id);
                      return (
                        <label
                          key={user.id}
                          className={`w-full flex items-center gap-4 rounded-2xl border p-4 transition-all ${selected
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="truncate text-gray-800 font-semibold">{fullName}</div>
                          </div>
                        </label>
                      );
                    })}
